...
class OauthApplicationsController < Doorkeeper::ApplicationsController
  ...
  before_action :set_application, only: %i[show edit update destroy regenerate_secret]

  def index
    @applications = scoped_applications.ordered_by(:created_at)
  end
  ...
  def create
    @application = scoped_applications.new(application_params)
  ...
  def scoped_applications
    Doorkeeper.config.application_model.where(owner: CurrentUser.user)
  end
