class TagBatchJob < ApplicationJob
  ...
  def perform(*args)
    ...
    normalized_antecedent = TagAlias.to_aliased(::Tag.scan_tags(@antecedent.downcase))
    normalized_consequent = TagAlias.to_aliased(::Tag.scan_tags(@consequent.downcase))
