...
RSpec.describe Staff::Post::DisapprovalsController do
  ...
  # ---------------------------------------------------------------------------
  # GET /staff/post/disapprovals
  # ---------------------------------------------------------------------------

  describe "GET /staff/post/disapprovals" do
    it "redirects anonymous to the login page" do
      get staff_post_disapprovals_path
      expect(response).to redirect_to(new_session_path(url: staff_post_disapprovals_path))
    end
    ...
    context "as approver requesting JSON" do
      ...
      it "returns a JSON array" do
        ...
        get staff_post_disapprovals_path(format: :json)
        expect(response).to have_http_status(:ok)
      ...
      it "filters by post_id search param" do
        ...
        get staff_post_disapprovals_path(format: :json), params: { search: { post_id: the_post.id } }
  ...
  # ---------------------------------------------------------------------------
  # POST /staff/post/disapprovals
  # ---------------------------------------------------------------------------

  describe "POST /staff/post/disapprovals" do
    let(:valid_params) do
      { post_disapproval: { post_id: the_post.id, reason: "other", message: "" } }
    end

    it "redirects anonymous to the login page" do
      post staff_post_disapprovals_path, params: valid_params
    ...
    context "as an approver" do
      ...
      it "creates a disapproval and returns JSON" do
        post staff_post_disapprovals_path(format: :json), params: valid_params
        expect(response).to have_http_status(:ok)
      ...
      it "is idempotent — a second POST for the same user and post does not create a duplicate" do
        post staff_post_disapprovals_path, params: valid_params
        expect do
          post staff_post_disapprovals_path, params: { post_disapproval: { post_id: the_post.id, reason: "borderline_quality", message: "updated" } }
        end.not_to change(PostDisapproval, :count)
      end

      it "updates reason and message on an existing record" do
        post staff_post_disapprovals_path, params: valid_params
        post staff_post_disapprovals_path, params: { post_disapproval: { post_id: the_post.id, reason: "borderline_quality", message: "updated note" } }
