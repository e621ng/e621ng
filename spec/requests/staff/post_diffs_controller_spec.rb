...
RSpec.describe Staff::PostDiffsController do
  ...
  describe "GET /moderator/post_diff" do
    ...
    it "redirects anonymous to the login page" do
      get staff_post_diff_path
      expect(response).to redirect_to(new_session_path(url: staff_post_diff_path))
    end
    ...
    context "as a janitor" do
      ...
      context "when only post_a is provided" do
        it "returns 200" do
          get staff_post_diff_path, params: { post_a: post_a.id }
          expect(response).to have_http_status(:ok)
        end
      end
      ...
      context "when only post_b is provided" do
        it "returns 200" do
          get staff_post_diff_path, params: { post_b: post_b.id }
          expect(response).to have_http_status(:ok)
        end
      end
      ...
      context "when both posts are identified by numeric ID" do
        it "returns 200" do
          get staff_post_diff_path, params: { post_a: post_a.id, post_b: post_b.id }
          expect(response).to have_http_status(:ok)
        end
      end
      ...
      context "when both posts are identified by MD5" do
        it "returns 200" do
          get staff_post_diff_path, params: { post_a: post_a.md5, post_b: post_b.md5 }
          expect(response).to have_http_status(:ok)
        end
      end
      ...
      context "when post_a is identified by a PostReplacement MD5" do
        ...
        it "returns 200" do
          get staff_post_diff_path, params: { post_a: replacement.md5, post_b: post_b.id }
          expect(response).to have_http_status(:ok)
        end
      end
      ...
      context "when a numeric ID does not match any post" do
        it "returns 404" do
          get staff_post_diff_path, params: { post_a: 0, post_b: post_b.id }
      ...
      context "when an MD5 does not match any post or replacement" do
        it "returns 200 without rendering the diff" do
          get staff_post_diff_path, params: { post_a: "deadbeefdeadbeefdeadbeefdeadbeef", post_b: post_b.id }
          expect(response).to have_http_status(:ok)
        end
      end
