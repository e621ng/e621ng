...
RSpec.describe Staff::UserCleanupsController do
  ...
  # ---------------------------------------------------------------------------
  # GET /staff/user_cleanups/:user_id
  # ---------------------------------------------------------------------------

  describe "GET /staff/user_cleanups/:user_id" do
    it "redirects anonymous to the login page" do
      get staff_user_cleanup_path(target)
      expect(response).to redirect_to(new_session_path(url: staff_user_cleanup_path(target)))
    end
  ...
  # ---------------------------------------------------------------------------
  # POST /staff/user_cleanups/:user_id/clear_avatar
  # ---------------------------------------------------------------------------

  describe "POST /staff/user_cleanups/:user_id/clear_avatar" do
    it "redirects anonymous to the login page" do
      post clear_avatar_staff_user_cleanup_path(target)
    ...
    context "as a moderator" do
      it "clears the user's avatar and redirects with a notice" do
        ...
        post clear_avatar_staff_user_cleanup_path(target)
        expect(response).to redirect_to(staff_user_cleanup_path(target))
  ...
  # ---------------------------------------------------------------------------
  # POST /staff/user_cleanups/:user_id/clear_profile
  # ---------------------------------------------------------------------------

  describe "POST /staff/user_cleanups/:user_id/clear_profile" do
    before { sign_in_as moderator }

    it "returns 403 for a member" do
      sign_in_as member
      post clear_profile_staff_user_cleanup_path(target)
      expect(response).to have_http_status(:forbidden)
    end
  ...
  # ---------------------------------------------------------------------------
  # POST /staff/user_cleanups/:user_id/hide_comments
  # ---------------------------------------------------------------------------

  describe "POST /staff/user_cleanups/:user_id/hide_comments" do
    before { sign_in_as moderator }
  ...
  # ---------------------------------------------------------------------------
  # POST /staff/user_cleanups/:user_id/hide_forum_posts
  # ---------------------------------------------------------------------------

  describe "POST /staff/user_cleanups/:user_id/hide_forum_posts" do
    before { sign_in_as moderator }
  ...
  # ---------------------------------------------------------------------------
  # POST /staff/user_cleanups/:user_id/hide_blips
  # ---------------------------------------------------------------------------

  describe "POST /staff/user_cleanups/:user_id/hide_blips" do
    before { sign_in_as moderator }
