...
RSpec.describe Staff::IpAddrsController do
  ...
  describe "GET /staff/ip_addrs" do
    it "redirects anonymous to the login page" do
      get staff_ip_addrs_path
      expect(response).to redirect_to(new_session_path(url: staff_ip_addrs_path))
    end
    ...
    context "as an admin" do
      ...
      context "when searching by user_name" do
        it "returns 200 HTML" do
          get staff_ip_addrs_path, params: { search: { user_name: target.name } }
          expect(response).to have_http_status(:ok)
        end

        it "returns 200 JSON in the correct format" do
          create(:comment, creator: target, creator_ip_addr: "127.0.0.1")
          get staff_ip_addrs_path(format: :json), params: { search: { user_name: target.name } }
          expect(response).to have_http_status(:ok)
      ...
      context "when searching by ip_addr" do
        it "returns 200 HTML" do
          get staff_ip_addrs_path, params: { search: { ip_addr: "127.0.0.1" } }
          expect(response).to have_http_status(:ok)
        end

        it "returns 200 JSON in the correct format" do
          create(:comment, creator: target, creator_ip_addr: "127.0.0.1")
          get staff_ip_addrs_path(format: :json), params: { search: { ip_addr: "127.0.0.1" } }
          expect(response).to have_http_status(:ok)
        ...
        it "returns 422 for invalid IP address" do
          get staff_ip_addrs_path, params: { search: { ip_addr: "*dylan*dolly*" } }
          expect(response).to have_http_status(:unprocessable_content)
        end

        it "returns 422 for invalid CIDR prefix" do
          get staff_ip_addrs_path, params: { search: { ip_addr: "127.0.0.1/999" } }
          expect(response).to have_http_status(:unprocessable_content)
        end
      end
    end
  ...
  # ---------------------------------------------------------------------------
  # GET /staff/ip_addrs/export
  # ---------------------------------------------------------------------------

  describe "GET /staff/ip_addrs/export" do
    it "redirects anonymous to the login page" do
      get export_staff_ip_addrs_path
      expect(response).to redirect_to(new_session_path(url: export_staff_ip_addrs_path))
    end
    ...
    it "returns 200 for an admin with no search params" do
      sign_in_as admin
      get export_staff_ip_addrs_path(format: :json)
      expect(response).to have_http_status(:ok)
    end
    ...
    context "as an admin" do
      ...
      context "when searching by user_name" do
        it "returns 200 JSON" do
          get export_staff_ip_addrs_path(format: :json), params: { search: { user_name: target.name } }
          expect(response).to have_http_status(:ok)
        end
      ...
      context "when searching by user_id" do
        it "returns 200 JSON" do
          get export_staff_ip_addrs_path(format: :json), params: { search: { user_id: target.id } }
          expect(response).to have_http_status(:ok)
        end
      ...
      context "when searching by ip_addr" do
        it "returns 200 and an empty array" do
          get export_staff_ip_addrs_path(format: :json), params: { search: { ip_addr: "127.0.0.1" } }
          expect(response).to have_http_status(:ok)
