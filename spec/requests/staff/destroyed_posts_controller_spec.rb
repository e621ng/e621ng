...
RSpec.describe Staff::DestroyedPostsController do
  ...
  # ---------------------------------------------------------------------------
  # GET /staff/destroyed_posts
  # ---------------------------------------------------------------------------

  describe "GET /staff/destroyed_posts" do
    it "redirects anonymous to the login page" do
      get staff_destroyed_posts_path
      expect(response).to redirect_to(new_session_path(url: staff_destroyed_posts_path))
    end
  ...
  # ---------------------------------------------------------------------------
  # GET /staff/destroyed_posts/:id
  # ---------------------------------------------------------------------------

  describe "GET /staff/destroyed_posts/:id" do
    let(:destroyed_post) { create(:destroyed_post) }

    it "redirects anonymous to the login page" do
      get staff_destroyed_post_path(destroyed_post.post_id)
      expect(response).to redirect_to(new_session_path(url: staff_destroyed_post_path(destroyed_post.post_id)))
    end
    ...
    it "redirects to the index filtered by post_id for an admin" do
      sign_in_as admin
      get staff_destroyed_post_path(destroyed_post.post_id)
      expect(response).to redirect_to(staff_destroyed_posts_path(search: { post_id: destroyed_post.post_id.to_s }))
    end
  end

  # ---------------------------------------------------------------------------
  # PATCH /staff/destroyed_posts/:id
  # ---------------------------------------------------------------------------

  describe "PATCH /staff/destroyed_posts/:id" do
    let(:destroyed_post) { create(:destroyed_post, notify: false) }

    it "redirects anonymous to the login page" do
      patch staff_destroyed_post_path(destroyed_post.post_id), params: { destroyed_post: { notify: "true" } }
    ...
    context "as bd_staff" do
      ...
      it "disables notifications and sets the correct flash message" do
        ...
        patch staff_destroyed_post_path(destroyed_post.post_id), params: { destroyed_post: { notify: "false" } }
      ...
      it "redirects to the index after update" do
        patch staff_destroyed_post_path(destroyed_post.post_id), params: { destroyed_post: { notify: "true" } }
        expect(response).to redirect_to(staff_destroyed_posts_path)
      end
