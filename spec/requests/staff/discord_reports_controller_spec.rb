...
RSpec.describe Staff::DiscordReportsController do
  let(:user)  { create(:user) }
  let(:admin) { create(:admin_user) }

  describe "GET /staff/discord_reports" do
    it "redirects anonymous to the login page" do
      get staff_discord_reports_path
      expect(response).to redirect_to(new_session_path(url: staff_discord_reports_path))
    end
