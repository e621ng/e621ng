...
class PostPrunerTest < ActiveSupport::TestCase
  def setup
    ...
    @old_post = FactoryBot.create(:post, :created_at => 30.days.ago, :is_pending => true)
    ...
    CurrentUser.scoped(@flagger, "127.0.0.2") do
      @unresolved_post_flag = FactoryBot.create(:post_flag, :created_at => 60.days.ago, :is_resolved => false, :post_id => @unresolved_flagged_post.id)
      @resolved_post_flag = FactoryBot.create(:post_flag, :created_at => 60.days.ago, :is_resolved => true, :post_id => @resolved_flagged_post.id)
    end
